...
def _pick_best_link(links):
    ...
    if isinstance(links, dict):
        lowered = {str(k).lower(): v for k, v in links.items() if v}
        for item in priority:
            if item in lowered:
                return lowered[item]
        return next(iter(lowered.values()), None)
    ...
    if isinstance(links, list):
        ...
        first = sorted_links[0]
        if isinstance(first, dict):
            return first.get("url") or first.get("downloadUrl") or first.get("link")
        return first
