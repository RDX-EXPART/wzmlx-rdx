...
# External universal bypass API used for auto-detect hosts like GDFlix, HubCloud and GoFile.
EXTERNAL_BYPASS_API = "https://tools.gkbotz.qzz.io".rstrip("/")
...
def _link_candidates(links):
    """Return mirror links in the best order. Used for GDFlix/HubCloud auto retry."""
    priority = (
        "instant dl",
        "fast cloud / zipdisk",
        "fast cloud",
    ...
    if isinstance(links, dict):
        items = [(str(k), v) for k, v in links.items() if v]
        lowered = {k.lower(): (k, v) for k, v in items}
        used = set()
        for item in priority:
            if item in lowered:
                name, link = lowered[item]
        ...
        for index, item in enumerate(sorted_links, 1):
            if isinstance(item, dict):
                link = item.get("url") or item.get("downloadUrl") or item.get("link")
                name = item.get("name") or item.get("type") or item.get("server") or f"Mirror {index}"
                if link:
                    candidates.append((str(name), link))
            elif item:
                candidates.append((f"Mirror {index}", item))

    return candidates


def _is_usable_link(link):
    """Light check only; no full download. If server blocks checks, keep the link."""
    if not isinstance(link, str) or not link.startswith(("http://", "https://")):
        return False
    try:
        res = Session().head(link, allow_redirects=True, timeout=8)
...
def _pick_best_link(links):
    candidates = _link_candidates(links)
    if not candidates:
        return None

    first_link = candidates[0][1]
    for _name, link in candidates:
        if _is_usable_link(link):
            return link
    return first_link


def _external_bypass(url, title="Bypass_File"):
    """Use one auto-detect API for supported hosts and normalize common responses."""
    try:
        data = get(f"{EXTERNAL_BYPASS_API}/api/bypass?url={quote(url, safe='')}").json()
    ...
    if not isinstance(data, dict):
        raise DirectDownloadLinkException("ERROR: Invalid bypass API response")

    if data.get("status") and data.get("status") not in ("success", "ok"):
        raise DirectDownloadLinkException(
            f"ERROR: {data.get('message') or data.get('error') or 'Bypass failed'}"
        )
    ...
    # GoFile-style multi response
    file_list = data.get("downloadLinks") or data.get("files") or data.get("list") or []
    if file_list:
        if len(file_list) == 1:
            file_ = file_list[0]
            if isinstance(file_, dict):
                single = (
                    file_.get("downloadUrl")
                    or file_.get("fast_download_link")
                    or file_.get("download_link")
                    or file_.get("url")
                    or _pick_best_link(file_.get("links", {}))
                )
                if single:
                    return single

        details = {"contents": [], "title": data.get("title") or title, "total_size": 0}
        for file_ in file_list:
            ...
            if not link:
                continue
            name = file_.get("name") or file_.get("filename") or title
            folder_path = (file_.get("folder") or file_.get("path") or "").strip("/")
            size = file_.get("bytes") or file_.get("size") or 0
            details["contents"].append({"path": folder_path, "filename": name, "url": link})
            details["total_size"] += _safe_int(size)
    ...
    raise DirectDownloadLinkException("ERROR: No downloadable link found from bypass API")
...
def direct_link_generator(link):
    ...
    elif any(x in domain for x in ["gdflix.net", "gdflix.dev", "gdflix.wtf", "gdflix.zip"]):
        return gdflix(link)
    elif any(x in domain for x in ["hubcloud.one", "hubcloud.foo", "hubcloud.ink", "hubcloud.art"]):
...
def gdflix(url):
    return _external_bypass(url, "GDFlix_File")


def hubcloud(url):
    return _external_bypass(url, "HubCloud_File")
...
def gofile_gk(url):
    return _external_bypass(url, "GoFile_File")


def gofile(url):
    return gofile_gk(url)
    if False: #bot_cache['on_vps']:
        _id = url.rsplit('/', 1)[-1]
